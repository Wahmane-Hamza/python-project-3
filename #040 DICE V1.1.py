...
## emoji() put in message_lbl randome emogi state valeu of state : "happy,laugh or cry"
def emoji(state):
...
def to_down(index_of):
    ...
    if tab_c_time[index_of] in tab_down:
        ...
        emoji('cry')
        on_other_player(index_of)
## place -3 by index of player ,if player stop on another player the second one will go back 3 steps
